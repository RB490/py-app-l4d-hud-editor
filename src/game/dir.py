    ...
    def _find_resource_recursive(self, current_dir, target_file, root_dir):
        for root, dirs, files in os.walk(current_dir):
            ...
            if target_file in files:
                ...
                log.info(f"Found '{target_file}' at: {resource_path}")
                return resource_path
        return None

    def get_resource_file_relative_path(self, resource_file):
        """
        Get the relative path of a resource file within the game's directories.
        This method exists because the header of resource files is not reliable eg: resource\hudlayout.res

        Args:
            resource_file (str): The name of the resource file to search for.

        Returns:
            str or None: The relative path of the resource file if found, or None if not found.

        Note:
            This method searches for the resource file in both the 'resource' and 'scripts' directories
            and their subdirectories.

        """
        log.info(f"Finding relative path for resource_file: {resource_file}")

        if not self.game.installation_exists(DirectoryMode.DEVELOPER):
            log.warning("Developer mode is not installed.")
            return None

        game_dir = self.game.dir.get(DirectoryMode.DEVELOPER)
        resource_dir = self._get_main_subdir(DirectoryMode.DEVELOPER, "resource")
        scripts_dir = self._get_main_subdir(DirectoryMode.DEVELOPER, "scripts")
        root_dir = self.game.dir.get_main_dir(DirectoryMode.DEVELOPER)

        resource_path = self._find_resource_recursive(os.path.join(game_dir, resource_dir), resource_file, root_dir)

        if resource_path is None:
            resource_path = self._find_resource_recursive(os.path.join(game_dir, scripts_dir), resource_file, root_dir)

        if resource_path is None:
            log.warning(f"Resource file '{resource_file}' not found.")
        else:
            log.info(f"Relative path for resource_file '{resource_file}': {resource_path}")
