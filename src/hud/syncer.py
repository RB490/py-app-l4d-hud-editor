...
class HudSyncer(metaclass=Singleton):
    ...
    def sync(self, source_dir: str, target_dir: str, target_dir_main_name: str) -> None:
        ...
        my_logger.info("Synching...")
        my_logger.info(f"Source: {source_dir}")
        my_logger.info(f"Target: {target_dir}")
        # my_logger.info(f"target_dir_main_name: {target_dir_main_name}")
        ...
        # input
        my_logger.info("")
        my_logger.info(f"Sync source_dir: {self.source_dir}")
        my_logger.info(f"Sync target_dir_root: {self.target_dir_root}")
        my_logger.info(f"Sync target_dir_main_name: {self.target_dir_main_name}")
        my_logger.info(f"Sync target_sub_dir_names: {self.target_sub_dir_names}")
        # my_logger.info(f"Sync hud_items: {self.hud_items}")
            ...
            for item in self.hud_items:
                ...
                if target_sub_dir_name == self.target_dir_main_name:
                    if not os.path.exists(target_item) and target_item not in self.hud_items_custom:
                        ...
                        my_logger.info(f"Adding custom file: {target_item}")

                # backup existing file
                # my_logger.info(f"loop hud item: {item}")
                if (
                    ...
                    try:
                        ...
                        my_logger.info(f"Backup: {target_item} -> {target_item_backup}")
                    except Exception as e:
                        # my_logger.info(f"Error backing up {target_item}: {e}")
                        raise Exception(f"Error backing up {target_item}: {e}") from e

        # my_logger.info(f"custom items: {self.hud_items_custom}")
            ...
            for item in self.hud_items:
                ...
                if os.path.isdir(item):
                    ...
                    if not os.path.isdir(target_item):
                        ...
                        my_logger.info(f"Creating: {target_item}")
                    continue
    ...
    def __unsync_item(self, item):
        ...
        for target_sub_dir_name in self.target_sub_dir_names:
            ...
            my_logger.info(f"Unsync directory: {target_sub_dir}")
            my_logger.info(f"& target_item_backup: {target_item_backup}")

            # delete custom directory
            if os.path.isdir(target_item) and target_item in self.hud_items_custom:
                shutil.rmtree(target_item)
                my_logger.info(f"Deleting custom directory: {target_item}")
            ...
            if os.path.isfile(target_item) and target_item in self.hud_items_custom:
                ...
                my_logger.info(f"Deleting custom file: {target_item}")
                self.hud_items_custom.remove(target_item)
                continue
            ...
            if os.path.isfile(target_item_backup):
                ...
                my_logger.info(f"Restoring: {target_item_backup} -> {target_item}")
