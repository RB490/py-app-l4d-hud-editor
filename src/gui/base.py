...
class BaseGUI:
    ...
    def run(self) -> None:
        ...
        my_logger.debug(f"Running {self.get_quoted_title()} with gui_type: {self.gui_type}")
    ...
    def __delayed_show(self, callback: str = ""):
        """After mainloop()"""
        my_logger.debug("Running __delayed_show")
    ...
    def get_hwnd(self):
        """Retrieve HWND"""
        my_logger.debug(f"{self.get_quoted_title()} HWND = {self.window_handle}")
    ...
    def set_transparency(self, transparency: float) -> None:
        ...
        my_logger.debug(f"{self.get_quoted_title()} Transparency set to {transparency}")
    ...
    def set_decorations(self, show_decorations: bool) -> None:
        ...
        if show_decorations:
            self.root.attributes("-fullscreen", False)
            my_logger.debug(f"{self.get_quoted_title()} Window decorations are now visible.")
        else:
            my_logger.debug(f"{self.get_quoted_title()} Window decorations are now hidden.")
    ...
    def set_window_geometry(self, geometry: str) -> None:
        ...
        try:
            ...
            my_logger.debug(f"Set {self.get_quoted_title()} to '{geometry}'!")
        except Exception:
            my_logger.exception(f"Error setting {self.get_quoted_title()} geometry")
    ...
    def get_window_geometry(self) -> str:
        ...
        if self.has_been_run:
            ...
            my_logger.debug(f"{self.get_quoted_title()} geometry: {geometry}")
            return geometry
        else:
            my_logger.warning(f"{self.get_quoted_title()} is NOT running. Returning default geometry.")
    ...
    def toggle_visibility(self):
        ...
        if self.is_hidden:
            ...
            self.hide()
        my_logger.debug(f"{self.get_quoted_title()} visibility toggled.")
    ...
    def __call_method_if_exists(self, method_name: str) -> None:
        if hasattr(self, method_name) and callable(getattr(self, method_name)):
            ...
            method()
            my_logger.debug(f"Called {method_name} for {self.get_quoted_title()}.")
        else:
            my_logger.debug(f"{self.get_quoted_title()} does not have a {method_name} method to call!")
    ...
    def show_menu_on_button(self, button, menu):
        ...
        my_logger.debug(f"Showing context menu @ x:{new_x} y:{new_y}")
