...
class Installer:
    ...
    def get_active_dir(self):
        ...
        if folder_path is None:
            raise Exception("Could not find any game installation directory")
    ...
    def get_dir(self, mode):
        ...
        message =   f"Could not find id file for the {mode} installation directory.\n\n\n\n" \
        ...
        if response == "No":
            return False
        folder_path = filedialog.askdirectory(mustexist=True, title=f"Select the {mode} directory", initialdir=self.steam_info.get("game_dir"))
    ...
    def _rebuild_audio(self):
        ...
        with open(valverc_path, 'w') as f:
            pass
            f.write("mat_setvideomode 800 600 1 0; snd_rebuildaudiocache; exit")
