...
class Game:
    ...
    def set_mode(self, mode):
        return self.installer.set_mode(mode)
    ...
    def run(self, type, wait_on_close=False):
        match type:
            case "user":
        ...
        if wait_on_close:
            # Run the command without waiting for it to finish (opening through steam means code would not connect to left4dead2.exe)
            exe_path = os.path.join(self.get_dir("dev"), self.game_exe)
            subprocess.run('"' + exe_path + '"' + game_args, shell=True)
        else:
